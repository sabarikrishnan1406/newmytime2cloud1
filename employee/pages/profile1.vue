...
export default {
  ...
  data: () => ({
    ...
    originalURL: `http://192.168.2.192:3000/register/visitor/`,
    fullLink: "",
  }),
  ...
  methods: {
    setLinkAndQrCode(e) {
      const { company_id, id: user_id, employee } = this.$auth.user;
      const branch_id = employee.branch_id;
      this.detail_data = e;
      let encrypted = this.$crypto.encrypt(`${company_id}-${e.id}-${branch_id}-${user_id}-${e.number}`)
      this.fullLink = `${this.originalURL}${encrypted}`;
