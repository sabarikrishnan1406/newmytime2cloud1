...
function parseRRuleString(input, targetTimezone, dtstart, strict = false) {
  ...
  if (/^DTSTART/im.test(unfoldedInput)) {
    ...
    const normalizedValueType = (valueType || ((dtValue == null ? void 0 : dtValue.includes("T")) ? "DATE-TIME" : "DATE")).toUpperCase();
    dtstartValueType = normalizedValueType === "DATE" ? "DATE" : "DATE-TIME";
    dtstartHasTzid = Boolean(dtTzid);
    dtstartIsUtc = Boolean(dtValue == null ? void 0 : dtValue.endsWith("Z"));
    ...
    switch (key.toUpperCase()) {
      ...
      case "UNTIL": {
        ...
        if (!untilHasTime) {
          ...
          const requiresUtc2 = dtstartHasTzid || dtstartIsUtc;
          opts.until = requiresUtc2 ? localEndOfDay.withTimeZone("UTC") : localEndOfDay;
          break;
        }
...
var _RRuleTemporal = class _RRuleTemporal {
  ...
  constructor(params) {
    ...
    if (isIcsOpts(params)) {
      ...
      this.tzid = (_c = (_b = parsed.tzid) != null ? _b : params.tzid) != null ? _c : "UTC";
      this.originalDtstart = _RRuleTemporal.normalizeToPolyfill(parsed.dtstart);
      manual = __spreadProps(__spreadValues({}, parsed), {
        // Allow explicit COUNT/UNTIL overrides when omitted from the RRULE string
        count: (_d = params.count) != null ? _d : parsed.count,
        until: (_e = params.until) != null ? _e : parsed.until,
      ...
      this.originalDtstart = _RRuleTemporal.normalizeToPolyfill(manual.dtstart);
    }
    if (!manual.freq) throw new Error("RRULE must include FREQ");
    manual.interval = (_f = manual.interval) != null ? _f : 1;
    ...
    if (manual.until) {
      manual.until = _RRuleTemporal.normalizeToPolyfill(manual.until);
    }
  ...
  hasTimeOfDayBetween(startTime, endTime) {
    var _a, _b, _c;
    if (import_polyfill.Temporal.PlainTime.compare(startTime, endTime) >= 0) return false;
    ...
    for (const hour of hours) {
      for (const minute of minutes) {
        for (const second of seconds) {
          const candidate = import_polyfill.Temporal.PlainTime.from({
            hour,
            minute,
            second,
          ...
          if (import_polyfill.Temporal.PlainTime.compare(candidate, startTime) >= 0 && import_polyfill.Temporal.PlainTime.compare(candidate, endTime) < 0) {
            return true;
          }
        }
      }
  ...
  between(after, before, inc = false) {
    ...
    if (tempOpts.count === void 0) {
      ...
      const dtstartNormalized = _RRuleTemporal.normalizeToPolyfill(this.opts.dtstart);
      const startZdtNormalized = _RRuleTemporal.normalizeToPolyfill(startZdt).withTimeZone(dtstartNormalized.timeZoneId);
      const alignedNormalized = _RRuleTemporal.normalizeToPolyfill(
      ...
      let candidate = _RRuleTemporal.normalizeToPolyfill(this.opts.dtstart.add(durationForJump(steps * interval)));
      if (steps > 0 && ["years", "months", "weeks", "days"].includes(unit)) {
        ...
        if (sameDate && import_polyfill.Temporal.ZonedDateTime.compare(candidate, startZdtNormalized) > 0) {
          if (this.hasTimeOfDayBetween(startZdtNormalized.toPlainTime(), candidate.toPlainTime())) {
            steps -= 1;
            candidate = _RRuleTemporal.normalizeToPolyfill(this.opts.dtstart.add(durationForJump(steps * interval)));
          }
        }
      }
      const dtstartForCompare = _RRuleTemporal.normalizeToPolyfill(this.opts.dtstart);
      if (import_polyfill.Temporal.ZonedDateTime.compare(candidate, dtstartForCompare) < 0) {
  ...
  occursOn(date) {
    const startOfDay = date.toZonedDateTime({
      timeZone: this.tzid,
      plainTime: import_polyfill.Temporal.PlainTime.from("00:00")
    });
...
function toText(input, locale, options = {}) {
  ...
  const resolvedLocale = locale != null ? locale : Intl.DateTimeFormat().resolvedOptions().locale;
  ...
  const textByDay = byDay != null ? byDay : freq === "WEEKLY" ? [weekdayTokenFromZdt(opts.dtstart)] : void 0;
  const textByHour = byHour != null ? byHour : shouldDefaultTime ? [opts.dtstart.hour] : void 0;
  const textByMinute = textByHour ? byMinute != null ? byMinute : [opts.dtstart.minute] : byMinute;
  const textBySecond = textByHour ? bySecond != null ? bySecond : [opts.dtstart.second] : bySecond;
  ...
  if (textByHour) {
    const minutes = textByMinute != null ? textByMinute : [0];
    const seconds = textBySecond != null ? textBySecond : [0];
  ...
  if (!textByHour && textByMinute) {
    parts.push(data.words.at_minute, list(textByMinute, void 0, data.words.and));
  }
  if (!textByHour && !textByMinute && textBySecond) {
    parts.push(data.words.at_second, list(textBySecond, void 0, data.words.and));
  }
