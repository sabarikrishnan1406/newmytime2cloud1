...
import { getBranches, createDevice, updateDevice, testCameraConnection, getDevice } from "@/lib/api";
...
const DeviceEdit = ({ open, setOpen, defaultPayload, onSuccess = () => { } }) => {
    ...
    useEffect(() => {
        if (open) {
            fetchBranches();
            // Fetch full device details with decrypted password
            if (defaultPayload && defaultPayload.id) {
                getDevice(defaultPayload.id).then(data => {
                    setForm(data);
                }).catch(error => {
                    // Fallback to defaultPayload if fetch fails
            ...
            >
                ...
                <div className={`absolute top-0 right-0 h-full w-full max-w-[520px] bg-white dark:bg-slate-800 shadow-2xl border-l border-gray-200 dark:border-white/10 transform transition-transform duration-300 ease-in-out flex flex-col ${open ? 'translate-x-0' : 'translate-x-full'}`}>
                    ...
                    <div className="px-6 py-5 border-b border-gray-200 dark:border-white/10 flex justify-between items-center shrink-0">
                        <div>
                            <h3 className="text-lg font-bold text-gray-600 dark:text-gray-300">Edit Device</h3>
