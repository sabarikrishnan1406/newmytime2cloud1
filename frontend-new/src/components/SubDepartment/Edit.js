...
import { createDepartment, createSubDepartments, getDepartments, updateSubDepartments } from "@/lib/api";
...
const EditSubDepartment = ({ defaultPayload, onSuccess = () => { } }) => {
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      let { data } = await updateSubDepartments(defaultPayload.id, form);
        ...
        >
          ...
          <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-white/10 w-full max-w-lg overflow-hidden transform transition-all animate-in fade-in zoom-in duration-200">
            ...
            <div className="px-6 py-5 border-b border-gray-200 dark:border-white/10 flex justify-between items-center">
              <div>
                <h3 className="text-lg font-bold text-gray-600 dark:text-gray-300">Edit Sub Department</h3>
                <p className="text-xs text-slate-400 mt-0.5">
                  update sub department in the system
                </p>
              </div>
              <button
                onClick={toggleModal}
