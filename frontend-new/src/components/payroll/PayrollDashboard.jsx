...
import {
  ...
  AlertCircle, Plus, Play, ThumbsUp, Download, FileText
} from "lucide-react";
import {
...
export default function PayrollDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const { data } = await api.get("/payroll-management/batches", { params });
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const bRes = await api.get("/payroll-management/batches", { params });
  ...
  return (
    <div className="space-y-5">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
              {batches.map(b => (
                <tr key={b.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition text-xs text-gray-600 dark:text-gray-300">
                  ...
                  <td className="px-4 py-3">{b.company_id}</td>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button className="inline-flex items-center gap-1.5 rounded-lg border border-gray-300 dark:border-white/10 bg-white dark:bg-gray-800 px-3 py-2 text-xs font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
          <FileText className="h-3.5 w-3.5" /> Export Register
        </button>
        <button className="inline-flex items-center gap-1.5 rounded-lg border border-gray-300 dark:border-white/10 bg-white dark:bg-gray-800 px-3 py-2 text-xs font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
